...
const getNewMovies = async (req, res) => {
    try {
        const newMovies = await Movie.find().sort({ createdAt: -1 }).limit(15);
        res.json(newMovies);
    } catch (error) {
        res.status(500).json({ error: error.message });
    }
...
const getTopMovies = async (req, res) => {
    try {
        const topRatedMovies = await Movie.find().sort({ numReviews: -1 }).limit(15);
...
const getRandomMovies = async (req, res) => {
    try {
        const randomeMovies = await Movie.aggregate([{ $sample: { size: 15 } }]);
        res.json(randomeMovies);
    } catch (error) {
        res.status(500).json({ error: error.message });
    }
}
